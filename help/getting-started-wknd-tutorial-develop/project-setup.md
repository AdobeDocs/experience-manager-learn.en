...
3. Add the **adobe-public** profile to the `settings.xml` file based on [the instructions here](https://helpx.adobe.com/experience-manager/kb/SetUpTheAdobeMavenRepository.html).
   ...
   A sample `settings.xml` is listed below or can be directly [downloaded](assets/chapter-1/settings-xml.txt). *Note, the naming convention of `settings.xml` is important.*
...
6. Paste the following into the command line to initiate the creation of a new project:
   ...
   > An alternative is to explicitly reference the granite archetype when generating the archetype:
...
11. Navigate from the **AEM Start** screen to **[Sites](http://localhost:4502/sites.html/content)**. The WKND Site will be one of the sites. It will include two content pages, one for English and one for French.
