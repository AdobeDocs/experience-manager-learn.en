...
1. Check out the `client-side-libraries/start` branch
...
1. Inspect the properties of `clientlibs/clientlib-base`.
...
1. Inspect the properties of `clientlibs/clientlib-grid`.
...
1. Create another file named `_elements.scss` beneath `src/main/webpack/base/sass` and populate it with the following:
...
1. Update `_shared.scss` beneath `src/main/webpack/base/sass` to include the `_elements.scss` and `_variables.scss` files.
...
1. Inspect the file `ui.frontend/clientlib.config.js`. This is the configuration file for an npm plugin, [aem-clientlib-generator](https://github.com/wcm-io-frontend/aem-clientlib-generator). **aem-clientlib-generator** is the tool responsible for transforming the compiled CSS/JavaScript and copy it into the **ui.apps** module.
...
1. Update `customheaderlibs.html` to include a reference to Google font styles that we specified earlier in the **ui.frontend** module. We will also comment out ContextHub for now...
...
1. Build and deploy the project to a local AEM instance using Maven:
...
1. Browse to the WKND Templates at [http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf/wknd](http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf/wknd).

1. Select and open the **Article Page Template** in the Template Editor.
...
1. Navigate to the **LA Skateparks** page created using the **Article Page Template**: [http://localhost:4502/editor.html/content/wknd/us/en/magazine/guide-la-skateparks.html](http://localhost:4502/editor.html/content/wknd/us/en/magazine/guide-la-skateparks.html). You should see a difference in fonts and some basic styles applied to indicate that the CSS created in the **ui.frontend** module is working.
...
1. Paste the copied markup from AEM into the `index.html` in the **ui.frontend** module beneath `src/main/webpack/static`.
...
1. Edit the `.scss` files and see the changes automatically reflected in the browser.
...
1. Check out the `client-side-libraries/solution` branch.
